...
이 프로젝트는 Kaggle에서 제공된 이커머스 데이터를 사용했습니다.

- 원본 데이터: [Kaggle 데이터셋 링크]
- 원본 CSV는 라이선스 및 파일 크기를 고려하여 저장소에 포함하지 않았습니다.
- 데이터를 내려받아 `data/raw/clean_final_data.csv`에 저장한 후 실행해주세요.
